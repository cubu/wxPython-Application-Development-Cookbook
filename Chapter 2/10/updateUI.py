...
class EditorUpdateUI(EditorWithToolBar):
    def __init__(self, parent, title=""):
        super(EditorUpdateUI, self).__init__(parent, title)
    ...
    def OnUpdateUI(self, event):
        ...
        if evtId in (wx.ID_COPY, wx.ID_CUT):
            event.Enable(self.txt.HasSelection())
...
class MyApp(wx.App):
    def OnInit(self):
        self.frame = EditorUpdateUI(None, title="Managing UI state")
        self.frame.Show();
        return True
