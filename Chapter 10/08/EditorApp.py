...
import wx.lib.softwareupdate as UPDATE

class MyApp(wx.App, UPDATE.SoftwareUpdate):
    def OnInit(self):
        self.SetAppDisplayName("Cookbook Text Editor")
        # Use local host as update url for example
        url = "http://127.0.0.1:8000"
        self.InitUpdates(url)
        self.CheckForUpdate(True)

        name = "TextEditor %s" % version.VERSION
        self.frame = TextEditorWithFind(None, title=name)
        self.frame.Show();
        return True
